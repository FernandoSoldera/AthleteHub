...
| AH-003 | Scaffold Flutter `client/` (pubspec, folders, theme + i18n skeleton, app shell) | TODO | AH-001 |
...
**Progress:** 1 done, 2 in progress / 47.

### Session log
- **2026-05-27** — AH-001 **DONE** (repo on `main`, docs flattened to `docs/`, root `.gitignore` + `README.md` + `CLAUDE.md`, `api/`+`client/`+`docs/`). AH-002 & AH-004 **WIP**: backend `api/` fully scaffolded (pom mirroring lotuga, Maven wrapper, `com.example.athletehub` layered packages, `AthleteHubApplication`, `application.properties` + `it` profile, Flyway baseline `V20260527120000`, `.env.example`, `Dockerfile`, `docker-compose.yml`, `ApiResponse`/`MessageCode`, exceptions + `GlobalExceptionHandler`, `WebConfig`/`JacksonConfig`, `SecurityConfig` skeleton, `AbstractIntegrationTest` + `SmokeIT`). **Remaining to close AH-002/004:** build-verify — needs **JDK 25** (machine `JAVA_HOME` is 24) + Docker, then `cd api && ./mvnw verify` (runs `SmokeIT`) and `./mvnw spring-boot:run` → `/actuator/health` UP. Profiles follow lotuga (single `application.properties` + env vars + `it` test profile), not separate dev/prod YAML. **Next:** AH-003 (Flutter scaffold), then EPIC 1.
